...
input_read_frame.place(x=0,y=260)

input_write_frame = Frame(window,bd=1,height=110 ,width=310)
input_write_frame.place(x=0,y=370)
...
def refreshImage(): #Threading
	...
	while True:
		if buffer_reg["RID"]:
			...
			register_read_value.set(readYUV422.readImage(port,regId=int(buffer_reg["RID"],16)))
			buffer_reg["RID"] = ""
		elif buffer_reg["WID"]:
			logging.debug("Write register")
			readYUV422.readImage(port,regId=int(buffer_reg["WID"],16),regVal=int(buffer_reg["VAL"],16))
			buffer_reg["WID"] = ""
			buffer_reg["VAL"] = ""
		else:
			logging.debug("Image requested")
			bitmap = readYUV422.readImage(port,width=cam_width,height=cam_height)
			index = 0
